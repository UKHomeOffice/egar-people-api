# API for Storing information on people for use in eGAR 
...
The system will connect to a db instance on the same host with the database `dev_people_api`.
...
```
mvn -Dflyway.url=jdbc:postgresql://localhost/dev_people_api -Dflyway.user=postgres -Dflyway.password=mysecretpassword flyway:migrate 
```
